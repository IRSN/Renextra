...
##'
##' The \code{"OT"} observations will be displayed has vertical
##' segments, while the \code{"MAX"} or \code{"OTS"} may be displayed
##' as horizontal segements when their date is not known. The columns
...
autoplot.Rendata <- function(object,
    ...
    if (!is.null(ou <- object$info$varUnit)) yLab <- sprintf("%s (%s)", yLab, ou)
